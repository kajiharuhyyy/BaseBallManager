...
public class GameService {
	...
	public List<Game> search(LocalDate gameDate, String opponent, LocalTime startTime, String ground,
		...
		return gameRepository.findAll().stream()
				...
				.filter(g -> opponent == null || opponent.isBlank()
						|| (g.getOpponent() != null && g.getOpponent().contains(opponent.trim())))
				.filter(g -> startTime == null || startTime.equals(g.getStartTime()))
				.filter(g -> ground == null || ground.isBlank()
						|| (g.getGround() != null && g.getGround().contains(ground.trim())))
				.filter(g -> myScore == null || (g.getMyScore() != null && g.getMyScore().equals(myScore)))
				.filter(g -> opponentScore == null
						|| (g.getOpponentScore() != null && g.getOpponentScore().equals(opponentScore)))
				.toList();
	}
